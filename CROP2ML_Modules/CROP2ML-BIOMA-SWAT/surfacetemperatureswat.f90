MODULE Surfacetemperatureswatmod_SW
    IMPLICIT NONE
CONTAINS

    SUBROUTINE model_surfacetemperatureswat_SW(GlobalSolarRadiation, &
    ...
    !                          ** unit : degC
        ...
        Tbare = Tavg + (Hterm * (AirTemperatureMaximum -  &
                AirTemperatureMinimum) / 2)
        WeightingCover = AboveGroundBiomass / (AboveGroundBiomass +  &
                EXP(7.563 - (0.0001297 * AboveGroundBiomass)))
        WeightingSnow = WaterEquivalentOfSnowPack /  &
                (WaterEquivalentOfSnowPack + EXP(6.055 - (0.3002 *  &
                WaterEquivalentOfSnowPack)))
        WeightingActual = MAX(WeightingCover, WeightingSnow)
        SurfaceSoilTemperature = WeightingActual *  &
                SoilTemperatureByLayers(1) + ((1 - WeightingActual) * Tbare)
    END SUBROUTINE model_surfacetemperatureswat_SW
