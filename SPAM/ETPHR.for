...
!     ------------------------------------------------------------------
      ...
      INTEGER ITER,NLAYR,J,YRDOY,DAS
...
C       Loop until evapotranspiration and photosynthesis are stable.
        ...
        IF (MEEVP.EQ.'Z' .AND. MEPHO.EQ.'L') THEN
          DO WHILE (REPEAT .AND. ITER .LE. 5)
            TCPREV = TCAN
! 08/10/2022 SC/FO - Argument list changed for EBL model.            
            CALL CANOPG(
     ...
     &        LFMXSH, LFMXSL, PCNLSH, PCNLSL,             !Output
     &        PGHR,PGSL,PGSH,                             !Output
     &        SLWREF, SLWSH, SLWSL, STRESS,               !Output
     &        AGEQESL,CO2QESL,QEFFSL,CISH,CISL,           !Output
     &        VPDSL,VPDSH)                                !Output
! 08/10/2022 SC/FO - Argument list changed for EBL model.     
            CALL CANPET(
...
! 08/10/2022 SC - Necessary if not use Gs control by VPD (i.e.,  Gs_code='OFF') 
...
! 08/10/2022 FO - This seems wrong. CSLPRV and CSHPRV should be set 
!               before CONDSL and CONDSH are calculated. Revisit.            
            CSLPRV = CONDSL
            CSHPRV = CONDSH
     ...
     &          RBSH, RBSL, RBSS)                         !Output
              ...
              ERRTR = ABS(THR-RWUH)
! 08/10/2022 SC/FO - Added source code for EBL model.              
! 08/10/2022 FO - Fixing a division by zero. Revisit.
              IF(THR-TPREV .NE. 0.0) THEN
                DIVTHR = (RWUH-TPREV)/(THR-TPREV)
              ELSE
                DIVTHR = 0.0
              ENDIF                
!             HOLD = CSLPRV+(CONDSL-CSLPRV)*MAX((RWUH-TPREV)/(THR-TPREV),.05)
              HOLD = CSLPRV+(CONDSL-CSLPRV)*MAX(DIVTHR,.05)
              CSLPRV = CONDSL
              CONDSL = MAX(1.0/RCUTIC,HOLD)
              
!             HOLD = CSHPRV+(CONDSH-CSHPRV)*MAX((RWUH-TPREV)/(THR-TPREV),.05)
              HOLD = CSHPRV+(CONDSH-CSHPRV)*MAX(DIVTHR,.05) 
              CSHPRV = CONDSH
...
! 08/10/2022 SC/FO - Following lines comment out by SC.
!        SWE = MAX(SWE-EHR,0.0)
...
!       the soil (1st and 2 layers) per time step        
        ...
        IF (SSH .LT. SWTD)THEN 
          !SWTD is the total deepth of SWE
          
          IF (H .EQ. 1) THEN
            !E can access only the first SSH cm's
            SWEH = SWE * (SSH/SWTD)
            SWE  = SWE - SWEH 
            SWEH2 = SWEH 
          ELSE
...
C=======================================================================
     ...
     &  VPDSL,VPDSH)                                      !Output
      ...
      CHARACTER TYPPGN*3,TYPPGL*3, GSCODE*3,MEEVP*1
...
! 08/10/2022 SC/FO - EBL model variables
      REAL RHUMHR,WINDHR,CICA,CISH,CISL,VPDSL,VPDSH,VPSAT
      REAL CISUN,CISUM,CINT
...
! 08/10/2022 SC/FO - EBL model initialization
...
! 08/10/2022 SC/FO - EBL model source code.
      GSCODE = 'ON'

      IF (GSCODE .EQ. 'OFF') THEN
...
! 08/10/2022 SC/FO - Updated subroutine for EBL model
...
C     Compute photosynthesis and leaf CO2 conductance for shaded leaves
     ...
     &      CONDSH, PGSH,CISH)                                !Output
      ...
      ENDDO !CIEFFECT=1,NINT 

! 08/10/2022 SC - Commented out: Need to do it in future. For now soil 
!               water is not send signal to stomata in the future, send 
!               this conductance increase effect under stress to impact 
!               CI when implment that we have to remove the impact bellow
...
C     Compute canopy photosynthesis (mmol CO2/m2/s).
      ...
      IF (STRESS) THEN
        IF (CONDSL .GT. 0.0) THEN
          IF(CSLSTR.GT.CONDSL) CSLSTR = CONDSL
        ...
        IF (CONDSH .GT. 0.0) THEN
          IF(CSHSTR.GT.CONDSH) CSHSTR = CONDSH 
...
C  12/01/90 NBP   Modified
C  11/05/93 KJB   Added layers for SLW and leaf N
C  08/10/22 SC/FO Added subroutine for EBL model.
C-----------------------------------------------------------------------
C  Called from: CANOPG
C  Calls:       CURV,TABEX
C  Notes : Standard conditions and suggested values for QEREF and LXREF.
...
C     from Ehleringer and Bjorkman (1977).  TEMPMX scaled to 1.0 at 30 oC.
      ...
      TEMPMX = TABEX(YLMAXT,XLMAXT,TEMPHR,6) /
     &         TABEX(YLMAXT,XLMAXT,30.0,6)
...
C     Calculate QEFF and LFMAX at ambient conditions.
      ...
      QEFF  = QEREF * CO2QE * AGEQE * BETAMX
      LFMAX = LXREF * SLWMAX * TEMPMX * AGEMXL * 
     &        CO2MAX * CHILL * BETAMX
...
C     CICA = 0.4+0.6*EXP(-0.002*CO2HR)
CSVC - the CO2 concentration that goes into conductance is the CS, 
!      CO2 at the leaf surface
      ...
      ENDIF
      CINT = CICA*CO2HR + (1.0-CICA)*GAMST
CSVC - in future implement equation to compute CS, for now assume 
!      it is equal to CA
      CINT = MAX(CINT,GAMST)


      IF (MEEVP .EQ. 'Z') THEN
C       gbCO2 from Pollard et al. (1995), eq. A39,  l = 0.07, 
C       assuming Wind Speed at leaf ~ 0.5 of  WINDHR
        gbco2 = MIN(10., MAX(0.1, (0.01* SQRT((WINDHR/2)/0.07)) *
     &           (1 / 0.029) )) 
C Leaf boundary layer co2 concentration
        CSHR = CO2HR - (PNLF/ gbco2)
        
C CSVC - when use gsmin equal to 0.01 model gets instabel for 
!        some experiments
...
CSVC 2021       CONDLF = 1.6 * CCO2LF * RT / PATM   !ORIGINAL 
...
! 08/10/2022 SC/FO - EBL variables
      REAL SHCAP(NL),DIFPR,EMISAV,TK4CAN,TK4SKY,STCONDSS
      REAL RSSH,RSSL,RSSS
...
! 08/10/2022 SC/FO - Added variables
      REAL VONK,DIFPR,TSURF(3,1),WINDC,VHCAIR,RASAC,STC,RIN,KS
...
C      IF(ZMD.LE.0.5)PRINT*,'WARNING: Canopy height < Ref. height
      ZMD = max(ZMD,0.5)
...
C       We should consider a correction based on thermal instability, 
C       applyed to all 3 RB's 
...
CSVC compute wind speed at the center of canopy
        ...
        !8.0 normalization constant for lower canopy aerodynamics
        xl =  MIN(1., 2.0 * XLAI / 4.0) 
        
        !Assumin that ztop-zbot is 0.9*H
        cvegl = (0.4 / MAX(0.9*H,0.01))*MAX(0.001,xl) 

        !diffusion coefficient, Assumin that ztop-zbot is 0.9*H
        dvegl = (0.1 * MAX(0.9*H,0.01)) / MAX(0.001,xl,xl**2)
...
c       taul = (ua(i) * VONK/(aloga(i)-alogu(i)))**2 * stramu(i) !taul = USTAR**2
        ...
        !Wind at the center of the Canopy
        WINDC =  MAX(0.01, SQRT(MAX(0.0, (cl/exphl+dl*exphl) )))

        !boundary considering wind at top of canopy
        RBSL = 1./ (0.01* SQRT((0.5*(WINDHR + WINDC))/0.07))
        
        !boundary considering wind at top of canopy
        RBSHb = 1./ (0.01* SQRT(WINDC/0.07))
     ...
     &           (1./ (0.01* SQRT(WINDSS/ZS0M))) 
       ...
       !Applying Correction for Thermal instability

        IF(WINDSP.LT.1.0) THEN
          IF((TSURF(1,1)-TCAN) .GT. 0.1) THEN
            RBSL = MIN(RBSL,VHCAIR/(1.52*ABS(TSURF(1,1)-TCAN)**(1./3.))) 
          ENDIF

          IF((TSURF(2,1)-TCAN) .GT. 0.1) THEN
            RBSH=MIN(RBSH,VHCAIR/(1.52*ABS(TSURF(2,1)-TCAN)**(1./3.))) 
          ENDIF
        
          IF((TSURF(3,1)-TCAN) .GT. 0.1) THEN
...
C=======================================================================
C  ETSOLV2, Subroutine, S. S. Jagtap, N.B. Pickering
C  Computes latent and sensible heat, and temperatures for zonal ET
C  model. Solves system of 10 equations using previous time step's
C  top-layer soil temperature.  Modified from Jagtap (1976), Jagtap
C  and Jones (1989).  Small errors in papers; equations here correct.
...
C  ??/??/89 SSJ Written
C  01/15/91 NBP Modified
C  01/07/20 SVC Modified
C  08/10/22  FO Added EBL model subroutine ETSOLV2
C-----------------------------------------------------------------------
C  Called from: ETPHR
C  Calls:       GAUSSJ,MATADD,MATCON,MATPRO,VPSLOP
C=======================================================================
...
C     VHAIRS and VSPIRL saved for later use.
     ...
     &  VHAIRS,VSPIRL,3,3,'+',                            !Input
     &  XMAT)                                             !Output (5)
      !+ STCND1/DZ1 !SVC where this STCND1/DZ1 came from???
      XMAT(3,3) = XMAT(3,3)
...
c      LHEAT(2,1) =  (VSP/RL(2,2))*(TSURF(2,1)-TAIRHR) + VPD(2,1)*VP/RL(2,2)
     ...
     &       + TSHR1*STCONDSS/DZ1 - VPD(3,1)*VP/RL(3,3) )
      ...
      VPIRLD(3,1) = HOLD + G 
      ! (TSURF(3,1)-TSHR1)*(STCONDSS/DZ1)!*(1./RS(3,3))  
      !*(TAIRHR-TSHR1)*(STCND1/DZ1)  ! Output (9)
...
C  08/10/22  FO Added SC and BK EBL model.
C-----------------------------------------------------------------------
C  Called from: CANPET
C  Calls:
C=======================================================================
     ...
     &  LAISHV,LAISLV,RADBK(3),EAIRHR,SBZCON,TAIRHR,TKAIR,
     &  TSKY,XLAI,RBKLF,RBACK,TCAN,ZERO
      REAL TSHR1 ,DIFPR,EMISAV,TK4CAN,TK4SKY
...
C     Update soil temperatures.
     ...
     &       TSHR(4),TSHR(5),TSHR(6),TSHR(7),TSHR(8),TSHR(9),
     &       TSHR(10),TSHR(11),TSHR(12),TA
